...
func Test_AWSCodeBuildProjectEncryptionNotDisabled(t *testing.T) {
 ...
 `, t)
		for _, result := range results {
			if result.Rule().LongID() == "aws-codebuild-enable-encryption" {
				assert.True(t, result.Severity() == severity.High, fmt.Sprintf("Result with code '%s' had wrong Severity reported '%s'", result.Rule().LongID(), result.Severity()))
			}
		}
	})
}
