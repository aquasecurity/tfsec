...
func init() {
	scanner.RegisterCheckRule(rule.Rule{
		...
		ShortCode: "defender-on-sql-servers",
...
`},
		...
		CheckFunc: func(set result.Set, resourceBlock block.Block, _ block.Module) {
			...
			if !resourceTypeAttr.Contains("SqlServers", block.IgnoreCase) {
				set.AddResult().
					WithDescription("Resource '%s' does not contain SqlServers", resourceBlock.FullName()).
