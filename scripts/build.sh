...
GOOS=linux GOARCH=amd64 go build -o bin/linux/${CHECK_GEN_BINARY}-linux-amd64 -ldflags "-X github.com/tfsec/tfsec/version.Version=${TAG}" ./cmd/tfsec-checkgen/
mkdir -p bin/windows
...
GOOS=windows GOARCH=amd64 go build -o bin/windows/${CHECK_GEN_BINARY}-windows-amd64.exe -ldflags "-X github.com/tfsec/tfsec/version.Version=${TRAVIS_TAG}" ./cmd/tfsec-checkgen/
